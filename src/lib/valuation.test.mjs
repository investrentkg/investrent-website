...
test('teksty v11.2: okres z numerem (T-f: umowa przed pulapem, kontakt = rozmowa/wiadomosc, nieodebrane nie licza sie, ponowne zgloszenie nie odnawia), T-g, T-h, T-e', async () => {
  ...
  assert.ok(items.includes('dowód zgody na telefon w sprawie wyceny (wersja zgody, kanał, czas): wygasa razem ze zgłoszeniem.'))
  assert.ok(last.indexOf('Wyjątek: umowa albo rozmowy o współpracy') > last.indexOf('najpóźniej 24 miesiące')); assert.ok(last.includes('Nieodebrane próby kontaktu z naszej strony okresów nie wydłużają'))
...
test('teksty v11.5: T-l2 (lista, Supabase UE), T-r, T-q, T-s, consentCall 3 drogi, wyjatek rozmowy o wspolpracy, kalendarz bez numeru, "sprawdzi"', async () => {
  ...
  assert.ok(!items.includes('Vercel opiera się') && !/Vercel[^.;]*(SCC|Privacy Framework|klauzul)/.test(items))
  ...
  assert.ok(items.includes('albo sam(a) poprosisz o rozmowy o współpracy')); assert.ok(how.includes('Wyjątek: umowa albo rozmowy o współpracy, o które sam(a) poprosisz'))
  assert.ok(!/nie pozwalaj\S* nam Cię zidentyfikować/.test(items + how)); assert.ok(how.includes('nie zawierają Twoich danych kontaktowych ani adresu IP'))
  const all = JSON.stringify(T)
