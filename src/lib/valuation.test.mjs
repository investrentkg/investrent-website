...
test('teksty v11.4: T-m (Railway 30 dni pierwsze), T-l (bez kopii DPF, bez Vercel/Brevo), T-o/T-n, doneBody, Anthropic SCC, Podczele tylko jako dzielnica', async () => {
  ...
  assert.ok(items.includes('Cloudflare i Google (dla kont w naszej domenie): Data Privacy Framework (ramy ochrony danych UE-USA), a w razie jego braku standardowe klauzule umowne UE zawarte w umowie powierzenia dostawcy'))
...
test('teksty v11.7: [4] jako lista odbiorcow (T-u) + osobny punkt zabezpieczen; Cenogram (Polska); lit. c w [2]; DPF objasniony; Vercel z umowa powierzenia', async () => {
  ...
  assert.ok(zab.items[0].includes('ramy ochrony danych UE-USA')); assert.ok(zab.items[1] === 'Vercel (hosting strony): Data Privacy Framework oraz umowa powierzenia przetwarzania danych;')
