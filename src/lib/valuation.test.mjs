...
test('notatka leada: dane, wynik, znacznik zgody, UTM (zgoda bez pelnych tekstow)', () => {
  ...
  assert.ok(n.startsWith('[Zgoda-kalkulator] kanał=telefon-wycena; czas=2026-09-25T10:00:00.000Z; wersja=wycena-2026-09-25-v10'))
  ...
  assert.ok(CONSENT_VERSION.endsWith('-v10') && !CONSENT_VERSION.includes('DO-PRAWNIKA'))
})
test('notatka leada: z zgoda marketingowa i dlugim UTM miesci sie w limicie 500 znakow backendu, znacznik zgody nieuciety', () => {
  ...
  assert.ok(n.startsWith('[Zgoda-kalkulator] kanał=telefon-wycena,marketing-telefon; czas=2026-09-25T10:00:00.000Z; wersja=wycena-2026-09-25-v10'))
...
test('teksty v8: zgoda marketingowa tylko telefon (bez SMS), okres marketingu wprost, errConsent w liczbie pojedynczej', async () => {
  ...
  assert.ok(all.includes('Po tym czasie nie będziemy już do Ciebie dzwonić w celach marketingowych')); assert.ok(T.lead.errConsent.endsWith('Zgoda marketingowa jest dobrowolna.'))
...
test('teksty v9: okres marketingu wg wariantu A, kopia dokumentu z zabezpieczeniami, IP okolo doby, Z3 w "Jak liczymy"', async () => {
  const { T } = await import('../app/wycena/texts.ts')
  const all = JSON.stringify(T)
  assert.ok(all.includes('rozmowa marketingowa tego okresu nie wydłuża')); assert.ok(all.includes('także jeśli nadal przechowujemy Twój numer z innego powodu'))
  assert.ok(!all.includes('wykonywalna')); assert.ok(!all.includes('Kopię zabezpieczeń')); assert.ok(all.includes('Kopię dokumentu z zabezpieczeniami transferu'))
  assert.ok(!all.includes('do 24')); assert.ok(all.includes('około 24'))
  assert.ok(T.how.body[T.how.body.length - 1].includes('Zgoda marketingowa (telefon) jest ważna do 12'))
})
test('teksty v10 (wariant B): skrot numeru tylko dla zgody marketingowej, oddzwonienie bez skrotu do wygasniecia zgloszenia, cofniecie zapisywane z data', async () => {
  const { T } = await import('../app/wycena/texts.ts')
  const items = T.lead.consentInfo.flatMap(c => [c.t, ...(c.items ?? [])]).join(' ')
  const all = JSON.stringify(T)
  assert.ok(items.includes('dowód zgody na telefon w sprawie wyceny (wersja zgody, kanał, czas; bez skrótu numeru)'))
  assert.ok(items.includes('dowód zgody marketingowej (jeśli ją zaznaczysz): 3'))
  assert.ok(!items.includes('dowód zgód: 3')); assert.ok(!items.includes('dowodu zgód opisanego'))
  assert.ok(items.includes('Datę cofnięcia zgody zapisujemy w naszym systemie')); assert.ok(items.includes('albo w rozmowie z pracownikiem biura'))
  assert.ok(items.includes('(dowód zgody marketingowej, adres IP;'))
  assert.ok(T.how.body[T.how.body.length - 2].includes('a dla zgody marketingowej także skrót numeru telefonu'))
  const last = T.how.body[T.how.body.length - 1]
  assert.ok(last.includes('dowód zgody na telefon w sprawie wyceny (bez skrótu numeru)') && last.includes('cofnięcie zapisujemy w naszym systemie z datą'))
  assert.ok(!all.includes('skrót numeru dla wszystkich'))
})
