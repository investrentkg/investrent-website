...
export default function KalkulatorPage() {
  ...
  const monthly = rate > 0
    ? loan * (rate / 100 / 12) / (1 - Math.pow(1 + rate / 100 / 12, -years * 12))
    : loan / (years * 12)
        ...
        <div style={{ background: 'linear-gradient(135deg, #0d2a5c 0%, #1a4fa0 100%)', padding: '56px 0 48px' }}>
          <div className="container">
            ...
            <p style={{ color: 'rgba(255,255,255,.7)', fontSize: 15, margin: 0 }}>
              Oblicz szacunkową ratę kredytu hipotecznego i sprawdź swoją zdolność kredytową
            </p>
          </div>
        </div>
        ...
        <div className="container" style={{ padding: '40px 0 80px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 28, maxWidth: 1000, margin: '0 auto' }}>
            ...
            <div style={{ background: 'white', borderRadius: 20, padding: 32, boxShadow: '0 2px 16px rgba(0,0,0,.06)' }}>
              ...
              {sliders.map(sl => (
                <div key={sl.label} style={{ marginBottom: 28 }}>
                  ...
                  <input type="range" min={sl.min} max={sl.max} step={sl.step} value={sl.value}
                    ...
                    style={{ width: '100%', accentColor: '#1a4fa0', cursor: 'pointer', height: 6 }} />
              ...
              <div style={{ background: '#eff6ff', borderRadius: 12, padding: '12px 16px', display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                <Info size={16} color="#1a4fa0" style={{ flexShrink: 0, marginTop: 1 }} />
                <p style={{ fontSize: 12, color: '#3b82f6', margin: 0, lineHeight: 1.6 }}>
                  Kalkulator ma charakter informacyjny. Dokładne warunki kredytu zależą od banku i Twojej historii kredytowej.
                </p>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 16 }}>
              ...
              {[
                ...
                <div key={item.label} style={{ background: 'white', borderRadius: 14, padding: '16px 20px',
                  display: 'flex', alignItems: 'center', gap: 14, boxShadow: '0 2px 12px rgba(0,0,0,.05)' }}>
              ...
              <a href="/kontakt"
                style={{ background: '#f5a623', color: 'white', borderRadius: 14, padding: '16px',
                  textAlign: 'center' as const, textDecoration: 'none', fontWeight: 700, fontSize: 15,
                  display: 'block', boxShadow: '0 4px 16px rgba(245,166,35,.35)', transition: 'transform .2s' }}
